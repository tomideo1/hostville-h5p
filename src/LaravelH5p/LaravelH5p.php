...
//            return storage('/h5p/libraries' . $path);
    ...
    private static function get_core_settings()
    {
        $settings = [
            ...
            // 'url'                => config('laravel-h5p.domain').'/api/h5p_protect?'.(Auth::check() ? 'token='.auth()->claims(['referer' => app('Illuminate\Routing\UrlGenerator')->previous()])->tokenById(1).'&' : '').'data=',//self::get_h5p_storage(), // for uploaded
...
//                $this->enqueue_assets($files);
        ...
        if (self::isRequestOk($request)) {
            ...
            try {
                ...
                // $params = json_decode($content['params']);
                    ...
                    DB::transaction(function () use (&$content,$oldLibrary,$params,$oldParams,&$editor,&$core){
                        $content['id'] = $core->saveContent($content);
                        $gamificationClass  = new LmsGamificationClass;
                        $gamificationClass->create($content['id']);
                    // Move images and find all content dependencies
                    $editor->processParameters($content['id'], $content['library'], $params, $oldLibrary, $oldParams);
    ...
    private function handleEdcCreation(){
